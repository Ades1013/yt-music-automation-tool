...
def obtener_lista_canciones():
    ...
    try:
        if ext == ".csv":
            ...
            if PyPDF2 is None:
                log(
                    "[X] Para leer PDFs necesitas instalar PyPDF2 (Abre la terminal y escribe: pip install PyPDF2)",
                    "error",
                )
            else:
            ...
            if docx is None:
                log(
                    "[X] Para leer archivos Word necesitas python-docx (Abre la terminal y escribe: pip install python-docx)",
                    "error",
                )
            else:
...
def ordenar_columna(tabla, col, reversa, ref_persistencia=None):
    ...
    tabla.heading(
        col, command=lambda: ordenar_columna(tabla, col, not reversa, ref_persistencia)
    )
...
def preguntar_opcion(cancion, opciones):
    ...
    tk.Label(
        dialogo, text="Hay varias opciones locales para:", font=("Segoe UI", 10)
    ).pack(pady=(10, 0))
    tk.Label(
        dialogo, text=f"'{cancion}'", font=("Segoe UI", 13, "bold"), fg="#0078D7"
    ).pack(pady=2)
    tk.Label(
        dialogo,
        text="Para descargar múltiples archivos, mantén presionada la tecla Ctrl o Shift al seleccionarlos.",
    ...
    tabla = ttk.Treeview(
        marco_tabla, columns=columnas, show="headings", selectmode="extended"
    )
    nombres_columnas = {
        "nombre": "Nombre",
    ...
    for col in columnas:
        tabla.heading(
            col,
            text=nombres_columnas[col],
            command=lambda c=col: ordenar_columna(
                tabla, c, False, persistencia_local_sort
            ),
        )
    ...
    if persistencia_local_sort["col"] in columnas:
        ordenar_columna(
            tabla,
            persistencia_local_sort["col"],
            persistencia_local_sort["rev"],
            persistencia_local_sort,
        )
...
def preguntar_opcion_yt(cancion, opciones_yt):
    ...
    tk.Label(
        dialogo, text="Audios Oficiales encontrados para:", font=("Segoe UI", 10)
    ).pack(pady=(10, 0))
    tk.Label(
        dialogo, text=f"'{cancion}'", font=("Segoe UI", 13, "bold"), fg="#0078D7"
    ).pack(pady=2)
    ...
    for col in columnas:
        tabla.heading(
            col,
            text=nombres_columnas[col],
            command=lambda c=col: ordenar_columna(tabla, c, False),
        )
    ...
    for op in opciones_yt:
        ...
        tabla.insert(
            "", tk.END, values=(titulo, canal, dur_str, tamano_est, vistas_str)
        )
    ...
    def tarea_hilo():
        ...
        try:
            ...
            if "ffmpeg" in error_msg.lower() or "ffprobe" in error_msg.lower():
                log(
                    f"[X] Falta 'ffmpeg.exe' en esta carpeta para poder convertir el audio de '{titulo_bajar}'.",
                    "error",
                )
            else:
...
def descargar_faltantes_ytdlp(lista_faltantes=None):
    faltantes_a_bajar = (
        lista_faltantes if lista_faltantes is not None else ultimas_faltantes
    )
    if not faltantes_a_bajar:
        messagebox.showinfo(
            "Sin faltantes", "No hay canciones pendientes para descargar."
        )
        return
    ...
    if not carpeta_destino:
        messagebox.showwarning(
            "Faltan datos", "Selecciona una carpeta destino primero."
        )
        return
    ...
    def tarea_ytdlp():
        ...
        for cancion in faltantes_a_bajar:
            ...
            try:
                ...
                if not opciones:
                    log(
                        f"[X] No se encontraron resultados oficiales para '{cancion}'.",
                        "error",
                    )
                    continue

                # Función para darle prioridad a los videos oficiales
                def es_oficial(op):
                ...
                # Ordenar por: 1. Es oficial (1 o 0), 2. Cantidad de vistas
                opciones.sort(
                    key=lambda x: (es_oficial(x), int(x.get("view_count") or 0)),
                    reverse=True,
                )
                ...
                if eleccion_yt:
                    for video in eleccion_yt:
                        ...
                        hilo_d = descargar_video(
                            url_bajar, titulo_bajar, ydl_opts_download
                        )
        ...
        log("==================================================\n", "info")
        messagebox.showinfo("Terminado", "El proceso de descarga ha concluido.")
...
def descargar_directo_youtube():
    ...
    if not canciones:
        messagebox.showwarning(
            "Faltan datos",
            "Por favor ingresa canciones o selecciona un archivo válido.",
        )
        return

    if not carpeta_destino:
        messagebox.showwarning(
            "Faltan datos", "Por favor selecciona la carpeta destino."
        )
        return
    ...
    def tarea_directa():
        ...
        for cancion in canciones:
            ...
            try:
                ...
                # Ordenar por: 1. Es oficial, 2. Vistas
                opciones.sort(
                    key=lambda x: (es_oficial(x), int(x.get("view_count") or 0)),
                    reverse=True,
                )
                ...
                for op in opciones:
                    ...
                    similitud = difflib.SequenceMatcher(
                        None, cancion_norm, titulo_op
                    ).ratio()

                    if (
                        similitud > 0.45
                        or cancion_norm.split("-")[0].strip() in titulo_op
                    ):
                        mejor_opcion = op
                        break
...
def procesar_canciones():
    ...
    if not canciones or not rutas_origen_cruda or not carpeta_destino:
        messagebox.showwarning(
            "Faltan datos",
            "Parcero, por favor revisa que las rutas y/o textos estén completos.",
        )
        return
        ...
        try:
            ...
            for cancion in canciones:
                ...
                for ruta_completa in archivos_locales:
                    ...
                    nombre_sin_num = re.sub(r"^\d+\s*[-_.]?\s*", "", nombre_base)
                    ...
                    if (
                        titulo_norm
                        and nombre_norm
                        and (titulo_norm in nombre_norm or nombre_norm in titulo_norm)
                    ):
                        match_cond = True
                    elif palabras_titulo:
                        palabras_nombre = set(
                            p for p in nombre_norm.split(" ") if len(p) > 2
                        )
                ...
                if len(coincidencias) == 0:
                    ...
                    destino = os.path.join(
                        carpeta_destino, os.path.basename(archivo_a_copiar)
                    )
                    ...
                    if eleccion_usuario and isinstance(eleccion_usuario, list):
                        for ruta_elegida in eleccion_usuario:
                            destino = os.path.join(
                                carpeta_destino, os.path.basename(ruta_elegida)
                            )
                            destino = obtener_nombre_unico(destino)
                            shutil.copy2(ruta_elegida, destino)
                            log(
                                f"[✓] Copiada (Manual): {os.path.basename(destino)}",
                                "exito",
                            )
                        total_encontradas += 1
                    else:
            ...
            log("\n==================================================", "info")
            log(
                f"★ Locales finalizadas | Encontradas: {total_encontradas} | Faltantes: {total_no_encontradas}",
                "exito",
            )
...
tk.Label(ventana, text="1. Canciones/Archivo:", font=fuente_etiquetas).grid(
    row=0, column=0, sticky="w", pady=5
)
entrada_lista = PlaceholderEntry(
    ventana,
    placeholder="1- Selecciona el archivo que contiene el nombre de las canciones ó escribe el nombre para descargarla",
    width=65,
    font=fuente_etiquetas,
)
entrada_lista.grid(row=0, column=1, padx=5, sticky="ew")
tk.Button(
    ventana, text="Buscar Archivo", command=seleccionar_archivo, font=fuente_etiquetas
).grid(row=0, column=2, columnspan=2, sticky="ew", padx=2)

tk.Label(ventana, text="2. Origen Local:", font=fuente_etiquetas).grid(
    row=1, column=0, sticky="w", pady=5
)
entrada_origen = PlaceholderEntry(
    ventana,
    placeholder="2- Selecciona la carpeta donde tienes tus canciones (Deja en blanco si quieres descargarlas)",
    width=65,
    font=fuente_etiquetas,
)
entrada_origen.grid(row=1, column=1, padx=5, sticky="ew")
tk.Button(
    ventana, text="Añadir Carpeta", command=seleccionar_origen, font=fuente_etiquetas
).grid(row=1, column=2, columnspan=2, sticky="ew", padx=2)

tk.Label(ventana, text="3. Carpeta Destino:", font=fuente_etiquetas).grid(
    row=2, column=0, sticky="w", pady=5
)
entrada_destino = PlaceholderEntry(
    ventana,
    placeholder="3- Selecciona la carpeta donde se guardaran tus canciones",
    width=65,
    font=fuente_etiquetas,
)
entrada_destino.grid(row=2, column=1, padx=5, sticky="ew")
tk.Button(
    ventana, text="Buscar", command=seleccionar_destino, font=fuente_etiquetas
).grid(row=2, column=2, columnspan=2, sticky="ew", padx=2)
...
boton_ejecutar = tk.Button(
    frame_acciones,
    text="¡Empezar a Copiar Locales!",
    bg="#2E8B57",
    fg="white",
    font=("Segoe UI", 10, "bold"),
    command=procesar_canciones,
    relief=tk.FLAT,
)
...
btn_ytdlp = tk.Button(
    ...
    fg="white",
    font=("Segoe UI", 10, "bold"),
    command=lambda: descargar_faltantes_ytdlp(ultimas_faltantes),
    relief=tk.FLAT,
)
btn_ytdlp.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(5, 5), ipady=5)

btn_yt_directo = tk.Button(
    frame_acciones,
    text="🚀 Descargar Directo",
    bg="#D32F2F",
    fg="white",
    font=("Segoe UI", 10, "bold"),
    command=descargar_directo_youtube,
    relief=tk.FLAT,
)
